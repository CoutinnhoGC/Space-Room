...
export function FilterBar({ instituicoes, tiposEspaco, value, onFilterChange, showInstitutionFilter = true }: FilterBarProps) {
  ...
  const resetFilters = () => onFilterChange({ institutionId: "TODAS", period: "7_DIAS", spaceType: "TODOS", status: "TODOS" });
  ...
  return (
    <div className="mb-6 rounded-xl border border-gray-100 bg-white p-4 dark:border-slate-800 dark:bg-slate-950">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        <div><label className="mb-1.5 block text-xs font-medium text-gray-600 dark:text-slate-400">Instituição</label><select value={value.institutionId} onChange={(event) => updateField("institutionId", event.target.value)} disabled={!showInstitutionFilter} className={inputClass}><option value="TODAS">{showInstitutionFilter ? "Todas as instituições" : "Instituição atual"}</option>{instituicoes.map((instituicao) => <option key={instituicao.idInstituicao} value={instituicao.idInstituicao}>{instituicao.nomeFantasia}</option>)}</select></div>
