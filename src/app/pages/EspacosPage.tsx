...
const emptyForm = { idEspaco: "", nome: "", descricao: "", tipo: "SALA" as TipoEspaco, capacidade: "", localizacao: "", idInstituicao: "", ativo: true };
...
export function EspacosPage() {
  ...
  const filtered = useMemo(() => espacos.filter((space) => (`${space.nome} ${space.localizacao ?? ""}`).toLowerCase().includes(search.toLowerCase()) && (tipoFiltro === "TODOS" || space.tipo === tipoFiltro)), [espacos, search, tipoFiltro]);
  ...
  const handleEdit = (space: Espaco) => {
    setForm({ idEspaco: String(space.idEspaco ?? ""), nome: space.nome, descricao: space.descricao ?? "", tipo: space.tipo, capacidade: String(space.capacidade), localizacao: space.localizacao ?? "", idInstituicao: String(space.idInstituicao), ativo: space.ativo !== false });
    setShowForm(true);
  };
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (form.idEspaco) {
        ...
        createNotification({ type: "ESPACO_CRIADO", institutionId: created.idInstituicao, title: "Novo espaço cadastrado", description: `${created.nome} foi adicionado${currentUser?.nome ? ` por ${currentUser.nome}` : ""}.`, entityId: created.idEspaco, actorUserId: currentUser?.idUsuario });
      ...
      <div className="mb-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">Gerencie todos os espaços cadastrados.</p>
          </div>
          <button onClick={() => { setShowForm(true); setForm({ ...emptyForm, idInstituicao: String(instituicoes[0]?.idInstituicao ?? "") }); }} className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-500 to-blue-600 px-4 py-2.5 text-white shadow-sm transition-all hover:from-blue-600 hover:to-blue-700">
        ...
        <div className="mb-6 rounded-xl border border-gray-100 bg-white p-6 dark:border-slate-800 dark:bg-slate-950">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <select value={form.idInstituicao} onChange={(event) => setForm((current) => ({ ...current, idInstituicao: event.target.value }))} className={inputClassName}>{instituicoes.map((instituicao) => <option key={instituicao.idInstituicao} value={instituicao.idInstituicao}>{instituicao.nomeFantasia}</option>)}</select>
            </div>
            <label className="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
              <input type="checkbox" checked={form.ativo} onChange={(event) => setForm((current) => ({ ...current, ativo: event.target.checked }))} />Espaço ativo
            </label>
            <div className="md:col-span-2">
              <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-slate-300">Descrição</label>
              <textarea value={form.descricao} maxLength={500} onChange={(event) => setForm((current) => ({ ...current, descricao: event.target.value }))} placeholder="Descrição" rows={4} className={`${inputClassName} md:col-span-2 w-full`} />
            </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="md:col-span-2">
            <div className="relative">
              ...
              <input value={search} maxLength={120} onChange={(event) => setSearch(event.target.value)} type="text" placeholder="Buscar espaços..." className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2 pl-10 pr-4 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100" />
            </div>
          </div>
          <select value={tipoFiltro} onChange={(event) => setTipoFiltro(event.target.value)} className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {!loading && filtered.map((space) => (
          <div key={space.idEspaco} className="rounded-xl border border-gray-100 bg-white p-5 transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-950">
            ...
            <div className="mb-3 flex items-center gap-4 text-sm text-gray-600 dark:text-slate-300"><div className="flex items-center gap-1.5"><Users className="h-4 w-4" /><span>{space.capacidade} pessoas</span></div></div>
