...
import { canChooseInstitution, canReserve, filterByActiveInstitution, isPlatformAdmin } from "../lib/permissions";
...
export function ReservasPage() {
  ...
  const currentFormInstitution = useMemo(() => instituicoes.find((item) => String(item.idInstituicao) === form.idInstituicao) ?? null, [instituicoes, form.idInstituicao]);
  ...
  const handleDelete = async (idReserva?: number) => {
    if (!idReserva || !window.confirm("Deseja confirmar a exclusão desta reserva?")) {
      return;
    }
  ...
  const handleCancel = async (reservation: Reserva) => {
    if (reservation.status === "CANCELADA") {
      toast.info("Esta reserva já está cancelada.");
      return;
    }
    ...
    try {
      setCancellingId(Number(reservation.idReserva));
      ...
      setSelectedReservation((current) => current?.idReserva === updated.idReserva ? updated : current);
    } catch (error) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (hasConflict) {
      toast.error(conflictDetails?.message || "Já existe uma reserva para esse espaço ou subespaço no intervalo selecionado.");
      return;
    }
    ...
    <>
      <div className="mb-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">Gerencie apenas as reservas da instituição ativa.</p>
          </div>
      ...
      {panelMode && (
        <div className="mb-6 rounded-xl border border-gray-100 bg-white p-6 dark:border-slate-800 dark:bg-slate-950">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-slate-100">{panelMode === "quick" ? "Reserva rápida" : form.idReserva ? "Editar reserva" : "Nova reserva"}</h2>
              <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">{panelMode === "quick" ? "Preencha os campos essenciais para registrar uma reserva imediata." : "Os conflitos entre pai e filhos são validados antes do envio."}</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              <input value={form.titulo} maxLength={150} onChange={(event) => setForm((current) => ({ ...current, titulo: event.target.value }))} placeholder="Título da reserva" className={inputClassName} />
            </div>
            <div>
            ...
            {showInstitutionSelector ? (
              <div>
                ...
                <select value={form.idInstituicao} onChange={(event) => setForm((current) => ({ ...current, idInstituicao: event.target.value, idEspaco: "", idSubespaco: "" }))} className={inputClassName}>{instituicoes.map((item) => <option key={item.idInstituicao} value={item.idInstituicao}>{item.nomeFantasia}</option>)}</select>
              </div>
            ) : (
              <div>
                <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-slate-300">Instituição</label>
                <div className={inputClassName}>{currentFormInstitution?.nomeFantasia ?? "Instituição atual"}</div>
              </div>
            )}
            <div>
              <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-slate-300">Responsável <span className="text-red-500">*</span></label>
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-slate-300">Espaço <span className="text-red-500">*</span></label>
            ...
            <div>
              ...
              <select value={form.idSubespaco} onChange={(event) => setForm((current) => ({ ...current, idSubespaco: event.target.value }))} className={inputClassName} disabled={availableSubspaces.length === 0}>
                <option value="">{availableSubspaces.length > 0 ? "Reservar o espaço completo" : "Sem subespaços cadastrados"}</option>
            ...
            <div><label className="mb-2 block text-sm font-medium text-gray-700 dark:text-slate-300">Data <span className="text-red-500">*</span></label><input type="date" min={minDate} value={form.data} onChange={(event) => setForm((current) => ({ ...current, data: event.target.value }))} className={inputClassName} /></div>
      ...
      <div className="mb-6 rounded-xl border border-gray-100 bg-white p-4 dark:border-slate-800 dark:bg-slate-950">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
          <div className="md:col-span-2"><div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400 dark:text-slate-500" /><input value={search} maxLength={120} onChange={(event) => setSearch(event.target.value)} type="text" placeholder="Buscar por espaço, subespaço, responsável ou título..." className="w-full rounded-lg border border-gray-200 bg-gray-50 py-3 pl-10 pr-4 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500" /></div></div>
          ...
          <select value={responsavelFiltro} onChange={(event) => setResponsavelFiltro(event.target.value)} className={inputClassName}><option value="TODOS">Todos os responsáveis</option>{usuarios.map((item) => <option key={item.idUsuario} value={item.idUsuario}>{item.nome}</option>)}</select>
          <select value={espacoFiltro} onChange={(event) => setEspacoFiltro(event.target.value)} className={inputClassName}><option value="TODOS">Todos os espaços</option>{getParentSpaces(espacos, currentUser?.idInstituicao).map((item) => <option key={item.idEspaco} value={item.idEspaco}>{item.nome}</option>)}</select>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
              {!loading && paginatedReservations.map((reservation) => {
                const usuario = usuarios.find((item) => item.idUsuario === reservation.idUsuario);
                const canCancelReservation = userCanReserve && reservation.status !== "CANCELADA";
                return <tr key={reservation.idReserva} className="transition-colors hover:bg-gray-50 dark:hover:bg-slate-900/60"><td className="px-6 py-4"><div className="text-sm font-medium text-gray-900 dark:text-slate-100">{getReservationSpaceLabel(reservation, espacos)}</div></td><td className="px-6 py-4"><div className="text-sm text-gray-700 dark:text-slate-300">{usuario?.nome ?? "Usuário não encontrado"}</div></td><td className="px-6 py-4"><div className="text-sm text-gray-700 dark:text-slate-300">{formatDate(reservation.dataInicio)}</div></td><td className="px-6 py-4"><div className="text-sm text-gray-700 dark:text-slate-300">{formatTimeRange(reservation.dataInicio, reservation.dataFim)}</div></td><td className="px-6 py-4"><span className={`rounded-md border px-2.5 py-1 text-xs font-medium ${getStatusReservaColor(reservation.status)}`}>{getStatusReservaLabel(reservation.status)}</span></td><td className="px-6 py-4"><div className="flex items-center justify-end gap-2"><button onClick={() => openDetails(reservation)} className="rounded p-1.5 text-gray-600 transition-colors hover:bg-gray-100 dark:text-slate-300 dark:hover:bg-slate-800" title="Visualizar detalhes" type="button"><Eye className="h-4 w-4" /></button>{userCanReserve && <button onClick={() => openEditReservation(reservation)} className="rounded p-1.5 text-blue-600 transition-colors hover:bg-blue-50 dark:text-blue-300 dark:hover:bg-blue-950/40" title="Editar reserva" type="button"><Edit2 className="h-4 w-4" /></button>}{canCancelReservation && <button onClick={() => handleCancel(reservation)} disabled={cancellingId === reservation.idReserva} className="rounded p-1.5 text-yellow-600 transition-colors hover:bg-yellow-50 disabled:opacity-50 dark:text-yellow-300 dark:hover:bg-yellow-950/40" title="Cancelar reserva" type="button"><XCircle className="h-4 w-4" /></button>}{platformAdmin && <button onClick={() => handleDelete(reservation.idReserva)} className="rounded p-1.5 text-red-600 transition-colors hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-950/40" title="Excluir reserva" type="button"><Trash2 className="h-4 w-4" /></button>}</div></td></tr>;
              })}
            </tbody>
          </table>
        </div>
      ...
      {selectedReservation && (
        <DetailPanel title={selectedReservation.titulo} subtitle="Visualização da reserva em modo de leitura." onClose={() => closePanels()}>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="rounded-xl border border-gray-100 bg-white p-4 dark:border-slate-800 dark:bg-slate-950"><div className="text-sm font-medium text-gray-900 dark:text-slate-100">Espaço principal</div><div className="mt-2 text-sm text-gray-600 dark:text-slate-300">{selectedReservationRelations.espaco?.nome ?? "Espaço não encontrado"}</div></div>
            <div className="rounded-xl border border-gray-100 bg-white p-4 dark:border-slate-800 dark:bg-slate-950"><div className="flex items-center gap-2 text-sm font-medium text-gray-900 dark:text-slate-100"><Layers3 className="h-4 w-4 text-blue-600 dark:text-blue-300" />Subespaço</div><div className="mt-2 text-sm text-gray-600 dark:text-slate-300">{selectedReservationRelations.subespaco?.nome ?? "Reserva do espaço completo"}</div></div>
