...
export function RelatoriosPage() {
  ...
  const reports = [
    ...
    { id: 2, title: "Reservas por instituição", description: "Resumo consolidado por instituição.", icon: BarChart3, color: "purple", onClick: () => { const lines = ["instituicao,total_reservas"]; instituicoesFiltradas.forEach((instituicao) => { const total = reservasFiltradas.filter((item) => item.idInstituicao === instituicao.idInstituicao).length; lines.push(`${instituicao.nomeFantasia},${total}`); }); downloadCsv("reservas-por-instituicao.csv", lines); toast.success("Relatório por instituição gerado."); } },
    ...
    { id: 4, title: "Espaços utilizados", description: "Uso dos espaços cadastrados.", icon: Calendar, color: "orange", onClick: () => { const lines = ["espaco,total_reservas"]; espacosFiltrados.forEach((espaco) => { const total = reservasFiltradas.filter((item) => item.idEspaco === espaco.idEspaco || item.idSubespaco === espaco.idEspaco).length; lines.push(`${espaco.nome},${total}`); }); downloadCsv("espacos-utilizados.csv", lines); toast.success("Relatório de espaços gerado."); } },
  ];
  ...
  return (
    <>
      ...
      {showInstitutionFilter && <div className="mb-6 rounded-xl border border-gray-100 bg-white p-4 dark:border-slate-800 dark:bg-slate-950"><div className="grid grid-cols-1 gap-4 md:grid-cols-3"><div><label className="mb-1.5 block text-xs font-medium text-gray-600 dark:text-slate-400">Instituição</label><select value={filtroInstituicao} onChange={(event) => setFiltroInstituicao(event.target.value)} className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"><option value="TODAS">Todas</option>{instituicoes.map((instituicao) => <option key={instituicao.idInstituicao} value={instituicao.idInstituicao}>{instituicao.nomeFantasia}</option>)}</select></div></div></div>}
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-3">
        ...
        <div className="rounded-xl border border-gray-100 bg-white p-6 dark:border-slate-800 dark:bg-slate-950"><div className="mb-1 text-3xl font-semibold text-gray-900 dark:text-slate-100">{espacosFiltrados.length}</div><div className="text-sm text-gray-500 dark:text-slate-400">Espaços cadastrados</div></div>
      </div>
    </>
  );
}
