...
export function UsuariosPage() {
  ...
  }, [usuarios, cargos, search]);

  const resetForm = () => {
    setForm({
      ...emptyForm,
      idCargo: String(cargos[0]?.idCargo ?? ""),
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Gerencie os usuarios do sistema</p>
          </div>
          <button
            onClick={() => {
              setShowForm(true);
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-100 p-6 mb-6">
          ...
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={handleSubmit}>
            ...
            <select value={form.idCargo} onChange={(event) => setForm((current) => ({ ...current, idCargo: event.target.value }))} className="px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg">
              {cargos.map((cargo) => (
